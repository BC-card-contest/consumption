...
## 왜 필요한가? 
최근 코로나가 장기됨에 따라 MZ세대에게 국내 여행의 증가와 코로나 종식이후 더 증가할 
여행 수요가 예상됨에 따라서 여행관련된 서비스가 있다면 좋겠다라고 생각이 들었다.
...
하지만 대부분 여행지는 막현하게 정한 반면에 여행지에서 무엇을 할것이고 무엇을 먹을지에 대해서 정하지 못했다는 것을 파악했다.
여행지에서 무엇을 하고 무엇을 먹을지에 대한 정보를 얻기 위해 포털 사이트에서 검색을 해보면 대부분
광고성 후기들이 대부분이여서 객관적인 정보를 통해 맛집 정보나 소위 말하는 핫플레이스 장소를
알아 내고 싶어하는 니즈를 발견하였고 이에 관한 서비스를 제공할 필요성을 느꼈다.

## 어떤것을 알려주는가? 
소비자들의 소비정보를 객관적인 데이터로 알려준다. 객관적인 데이터를 제공한다는 말은 실제 결제 데이터 즉 한 장소에서 
얼마나 많은 결제가 이루어졌나를 토대로 실제로 사람들이 어느 시간대에 어디를 많이 가고 어느 가게에서 소비를 많이 하는지를 
제공하고 이러한 데이터를 토대로 여행 계획을 짜는데 도움을 줄 수 있다고 생각한다.
...
작은 동그라미로 표시하여 어떠한 곳에서 많은 결제가 일어나고 있는지 사용자가 한 눈에 파악할 수 있도록 도움을 준다. <br>
...
이와 같이 어느 가게에서 결제가 많이 일어나고 있는지에 대한 정보만 제공하는 것이 
아닌 어느 시간대에 결제가 많이 일어나는지에 대한 정보를 제공하여 이용자가 사람들이 별로 없는 
시간대에 방문할 수 있도록 도움을 주거나 이용자와 동일 성별은 가진 사람이나 비슷한 연령대의 사람들이
어떠한 장소를 자주 방문하는지에 대한 정보와 같이 자세한 정보를 제공할 수 있다.
...
또한 특정 업계에 국한된 것이 아니라 결제가 이루워지는 모든 업계에 데이터를 가져올 수 있어 더 많은 방식의 데이터 활용을 할 수 있을 것 같다.
